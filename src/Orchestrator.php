...
class Orchestrator implements LoggerAwareInterface
{
    ...
    public function processTask(string $userStory): TaskResult
    {
        ...
        $this->logger->info("Geplante Dateien: " . implode(', ', $plan['files']));
        $generatedFiles = [];
        $lastResponse   = null;
        ...
        foreach ($plan['files'] as $fileName) {
            ...
            while ($round < $maxRounds) {
                ...
                $this->logger->info("[Coder-Response]: {$response}");
    ...
    private function saveGeneratedFile(
        ...
        if (str_ends_with($fileName, '.php')) {
            ...
            $this->logger->info("[Lint] " . trim($lintResult));
        }
        return $fullPath;
    }
